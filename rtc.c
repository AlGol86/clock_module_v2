...
void init_rtc(void) {
  GPIOC->CR2&=~BIT_clk_rtc ; // low speed (interrupt disabled)
  GPIOA->CR2&=~BIT_rst_rtc ; // low speed (interrupt disabled)
  GPIOC->CR1|=BIT_clk_rtc ; // p-p 
  GPIOA->CR1|=BIT_rst_rtc ; // p-p 
  GPIOC->ODR&=~BIT_clk_rtc ;  //clk_pin, rst_pin  - 0 
  GPIOA->ODR&=~BIT_rst_rtc ;  //clk_pin, rst_pin  - 0 
  GPIOC->DDR|=BIT_clk_rtc ; 
  GPIOA->DDR|=BIT_rst_rtc ; 
   
  GPIOC->CR2&=~BIT_dat_rtc; //dat_pin  - low speed (interrupt disabled)
  GPIOC->CR1|=BIT_dat_rtc; //dat_pin - p-p /float in inp. direct
  GPIOC->ODR&=~BIT_dat_rtc;  //dat_pin  - 1 (SDA)
  GPIOC->DDR|=BIT_dat_rtc; 
 }
...
void send_byte_rtc(char data) {
	char i;
  for(i=1;i<=8;i++) {
    GPIOC->ODR|=(data&1)<<PIN_dat_rtc;
    data>>=1;
    GPIOC->ODR|=BIT_clk_rtc; // clk=1
    nop();
    GPIOC->ODR&=~BIT_dat_rtc; // sda=1
    GPIOC->ODR&=~BIT_clk_rtc;// clk=0
  }
}
...
char  receive_byte_rtc(void) {
  ...
  for(i=0;i<=7;i++) {
    ...
    GPIOC->ODR|=BIT_clk_rtc; // clk=1
    nop();
    data|=((GPIOC->IDR&BIT_dat_rtc)>>PIN_dat_rtc)<<i;
    nop();
    GPIOC->ODR&=~BIT_clk_rtc;// clk=0
    nop();
    }
...
void send_rtc(char reg_adr, char data) { 
  ...
  delay_rtc(3); //9*0.5us 
  GPIOA->ODR|=BIT_rst_rtc; //rst=1(>4us)
  delay_rtc(3); //9*0.5us 
  reg_adr<<=1;
  reg_adr+=0x80;
